...
public class RoundProgressBar extends TextView {
	...
	private int mSecondaryColor; // 圆形区域颜色
	...
	private int mTimerInterval; // 定时器触发间隔时
	...
	private final static String MODE_REPEAT = "repeat";

	@SuppressWarnings("unused")
	private long mCurTime;
	
	private String mMode; // 模式
	...
	public RoundProgressBar(Context context, AttributeSet attrs) {
		...
		mPaintWidth = array
				.getInt(R.styleable.RoundProgressBar_Paint_Width, 10);
...
//		int color = getResources().getColor(R.color.pt_gray);
		mSecondaryColor = array.getColor(R.styleable.RoundProgressBar_Secondary_Paint_Color,
				0xffffcc00);
		mSecondaryPaint.setColor(mSecondaryColor);
		
		mMode = array.getString(R.styleable.RoundProgressBar_mode);
		if (MODE_REPEAT.equals(mMode)) {
			mMaxProgress = Integer.MAX_VALUE;
		}
		...
		mHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
				...
				if (msg.what == TIMER_ID) {
					...
					if (MODE_REPEAT.equals(mMode)) {
						mCurFloatProcess += 360 / 10 / 6;
						mCurFloatProcess = mCurFloatProcess > 360 ? mCurFloatProcess % 360 : mCurFloatProcess;
	...
	public synchronized void setProgress(int progress) {
		...
		if (!MODE_REPEAT.equals(mMode) && mCurProgress > mMaxProgress) {
...
//		float secondSweep = 360 * secondRate;
		canvas.drawArc(mRoundOval, mStartProgress, 360f,
				mBRoundPaintsFill, mSecondaryPaint);

		if (MODE_REPEAT.equals(mMode)) {
			canvas.drawArc(mRoundOval, mCurProgress, 360 / 10, mBRoundPaintsFill,
					mRoundPaints);
//			LogUtil.d("RoundProgressBar", "mCurProgress : " + mCurProgress);
		} else {
